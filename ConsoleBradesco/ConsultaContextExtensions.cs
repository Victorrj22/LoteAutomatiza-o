...
public static class ConsultaContextExtensions
{
    ...
    {
        // Cria um novo TokenAcesso
        var sqlQuery1 = "INSERT INTO public.token_acesso (cliente_id, token, acesso_total, incluido_em, incluido_por, name, key_type) " +
                       "VALUES (1631, translate(uuid_generate_v4()::TEXT, '{{}}-', ''), true, now(), 0, 'Bradesco RH Lote 2023-12-07 (114)', 1)";
        
        context.Database.ExecuteSqlRaw(sqlQuery1);
        context.SaveChanges();
    }

    public static void AdicionarConsultaPorSQL(this ConsultaContext context)
    {
        using (var transaction = context.Database.BeginTransaction())
        {
            try
            {
                var sqlQuery = @"INSERT INTO public.consulta
(consulta_tipo_id,
 token_acesso_id,
...
SELECT ct.consulta_tipo_id,
       ...
       FALSE                                                                      retorno_original_apagado,
       FALSE                                                                      retorno_desatualizado,
       '{
         ""ExecutionMode"": 1,
         ""Async"": true,
      ...
      FROM (
               SELECT DISTINCT
                   ON (cpf) linha,
                            ...
                            NULL                                                                                    email,
                            a6a383bf3f74438799d09212d15e26ec                                                                             token_acesso_id
               FROM customer_bradesco_rh_jobs.lote_20231207
